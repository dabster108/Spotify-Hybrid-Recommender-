...
try:
    ...
    print("⚠️ Advanced Query Analyzer not available, using fallback interpretation")
...
class HybridRecommendationSystem:
    def __init__(self):
        ...
        self.groq_api_key = "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"
        ...
        if ANALYZER_AVAILABLE:
            ...
            print("⚠️ Using basic fallback interpretation")
...
Only return valid JSON, no explanations."""
            ...
            if response.status_code == 200:
                ...
                if json_match:
                    ...
                    return UserPreferences(
                        genres=prefs_dict.get('genres', ['pop']),
                        moods=prefs_dict.get('moods', ['neutral']),
    ...
    def _fallback_query_interpretation(self, query: str) -> UserPreferences:
        ...
        if self.query_analyzer:
            try:
                ...
                print(f"⚠️ Advanced analyzer failed: {e}, using basic fallback")
    ...
    def search_spotify_tracks(self, preferences: UserPreferences, limit: int = 100, specific_artist: str = None) -> List[Track]:
        ...
        if specific_artist:
            ...
            search_queries.extend(preferences.moods[:3])
    ...
    def hybrid_merge(self, sequential_results: List[Tuple[Track, float]], 
        ...
        # SPECIFIC ARTIST REQUEST - BYPASS DIVERSITY ENFORCEMENT
        if specific_artist:
            print(f"🎤 Bypassing artist diversity for {specific_artist} - including all relevant tracks")
            ...
            for track_id, score in sorted_tracks:
                ...
                # Check if requested artist is exactly one of the track's artists or featured
                artist_match = False
                for artist in track_artists:
                    # Exact match or the requested artist is the main part of the artist name
                    if (artist == specific_artist_lower or 
                        artist.startswith(specific_artist_lower + " ") or
                        artist.endswith(" " + specific_artist_lower) or
                        specific_artist_lower in artist.split(" feat") or
                        specific_artist_lower in artist.split(" featuring") or
                        specific_artist_lower in artist.split(" with") or
                        specific_artist_lower in artist.split(" & ")):
                        artist_match = True
                        break
    ...
    def evaluate_recommendations(self, recommendations: List[Track], ground_truth: List[str] = None) -> Dict[str, float]:
        ...
        # Diversity metrics
        artists = [track.artists[0] for track in recommendations if track.artists]
        unique_artists = len(set(artists))
        metrics['artist_diversity'] = unique_artists / len(recommendations) if recommendations else 0
        
        # Popularity distribution
        popularities = [track.popularity for track in recommendations]
        if popularities:
        ...
        # Audio feature diversity (if available)
        if recommendations and hasattr(recommendations[0], 'energy'):
            energy_values = [track.energy for track in recommendations if hasattr(track, 'energy')]
            valence_values = [track.valence for track in recommendations if hasattr(track, 'valence')]
        ...
        # Novelty (how recent are the tracks)
    ...
    def get_hybrid_recommendations(self, query: str, existing_songs: List[str] = None, specific_artist: str = None, requested_count: int = None) -> str:
        ...
        if not candidate_tracks:
            if specific_artist:
                return f"❌ No tracks found for artist '{specific_artist}'. Please check the artist name and try again."
        ...
        if specific_artist:
            # For artist-specific: Focus on ranking by popularity and audio features
            print("   Using artist-focused ranking (no diversity enforcement)")
            sequential_results = self.sequential_recommendations(enhanced_tracks, top_k=20)
            ranking_results = self.ranking_recommendations(enhanced_tracks, preferences, top_k=20)
            embedding_results = self.embedding_recommendations(enhanced_tracks, query, top_k=20)
        else:
        ...
        for i, track in enumerate(tracks, 1):
            ...
            if specific_artist:
                if len(track.artists) > 1:
                    collaboration_indicator = " 🤝"  # Collaboration emoji
